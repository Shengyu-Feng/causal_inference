...
class VAE(object):
  ...
  def _create_recognition_network(self, x, reuse=False):
    with tf.variable_scope("rec", reuse=reuse) as scope:
      ...
      W_fc1, b_fc1     = self._fc_weight_variable([5*5*32, 256], "fc1")
      W_fc2, b_fc2     = self._fc_weight_variable([256, 256], "fc2")
      W_fc3, b_fc3     = self._fc_weight_variable([256, 30],  "fc3")
      W_fc4, b_fc4     = self._fc_weight_variable([256, 30],  "fc4")
      ...
      h_conv4_flat = tf.reshape(h_conv4, [-1, 5*5*32])
  ...
  def _create_generator_network(self, z, reuse=False):
    with tf.variable_scope("gen", reuse=reuse) as scope:
      W_fc1, b_fc1 = self._fc_weight_variable([30,  256],    "fc1")
      W_fc2, b_fc2 = self._fc_weight_variable([256, 5*5*32], "fc2")
      ...
      h_fc2 = tf.nn.relu(tf.matmul(h_fc1, W_fc2) + b_fc2)
      h_fc2_reshaped = tf.reshape(h_fc2, [-1, 5, 5, 32])
      ...
      x_out_logit = tf.reshape(h_deconv4, [-1, 80*80*1])
      return x_out_logit
  ...
  def _create_loss_optimizer(self):
    # Reconstruction loss
    reconstr_loss = tf.square(self.x - self.x_out)
