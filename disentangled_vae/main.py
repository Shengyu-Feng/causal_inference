...
tf.app.flags.DEFINE_integer("epoch_size", 800, "epoch size")
...
def reconstruct_check(sess, model, images):
  ...
  for i in range(len(images)):
    org_img = images[i].reshape(80, 80)
    org_img = org_img.astype(np.float32)
    reconstr_img = x_reconstruct[i].reshape(80, 80)
...
def disentangle_check(sess, model, manager, save_original=False):
  ...
  if save_original:
    imsave("original.png", img.reshape(80, 80).astype(np.float32))
  ...
  for target_z_index in range(n_z):
    for ri in range(n_z):
      ...
      rimg = reconstr_img[0].reshape(80, 80)
